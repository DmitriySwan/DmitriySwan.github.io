...
$(document).ready(function () {
    // breakpoints
    var mql1024 = window.matchMedia("(min-width:1024px)"),
        mql768 = window.matchMedia("(min-width:768px)"),
        mql640 = window.matchMedia("(min-width:640px)");

    // Фикс для ховер эффектов на мобильных
    ...
    var disableScroll = function() {
        $('body').css('overflow', 'hidden')
            .on('touchmove', preventFunction);
    };

    var enableScroll = function() {
        $('body').css('overflow', '')
            .off('touchmove', preventFunction);
    };
    ...
    var leftArrow = $('.fotorama__arr--prev');
    var rightArrow = $('.fotorama__arr--next');
    ...
    // перемещаем стрелки
    var fotoramaNav = $('.fotorama__nav__shaft');
    var fotoramaFrame = $('.fotorama__stage__shaft');

    fotoramaNav.prepend(leftArrow);
    fotoramaNav.append(rightArrow);

    // создаем прямоугольники которые будут псевдо фоном для стрелок и перемещаем
    // var rectanglePrev = document.createElement('DIV');
    // var rectangleNext = document.createElement('DIV');
    //
    // rectanglePrev.classList.add('fotorama-rectangle', 'fotorama-rectangle__prev');
    // rectangleNext.classList.add('fotorama-rectangle', 'fotorama-rectangle__next');
    //
    // fotoramaNav.prepend(rectanglePrev);
    // fotoramaNav.append(rectangleNext);

    // определение слайдеров
    var sliderFrame = $('.fotorama__nav__frame');

    // текущий
    fotoramaFrame.on('touchend', 'click', function () {
        getNumberActiveElem(sliderFrame);
    });

    sliderFrame.on('click', function () {
        getNumberActiveElem(sliderFrame);
    });
    ...
    function getNumberActiveElem(arr) {
        arr.each(function( index ) {
            var elem = $(this);

            if (elem.hasClass('fotorama__active')) {
                $('.js-current').text('0' + (index + 1));
            }
        });
    }

    // всего
    $('.js-all').text('0' + sliderFrame.length + '-');


    // плеер
    var player = $('.player');
    var play = $('.player_play-icon');
    var pause = $('.player_pause-icon');
    var tab = $('.player-categories_tab');
    var button = $('.player_play-button');
    var sections = $('.player-content_section');

    var playerState = 'player__state-pause';


    // play
    play.on('click', function () {
        var parent = $(this).closest('.player-content_item');
        var audio = parent.find('audio')[0];
        var duration = parent.find('.player_duration');
        var audioType = 'audio/mp3';

        if (!audio.canPlayType(audioType)) {
            alert('К сожалению не удается воспроизвести аудио. Попробуйте в другом браузере!');
            return false;
        }

        player.find(button).removeClass(playerState);
        player.find('audio').each(function () {
            var elem = $(this)[0];

            // если музыка проигрывается и воспроизведение не закончилось
            if (!elem.paused && !elem.ended) elem.pause();
        });

        parent.find(button).addClass(playerState);
        audio.play();
    ...
    pause.on('click', function () {
        ...
        parent.find(button).removeClass(playerState);
        audio.pause();
        window.clearInterval(updateTime);
    });

    // текущее время
    function update(audio, elem) {
        console.log('я выполняюсь!');
        if (!audio.ended) {
            var playedS = parseInt(audio.currentTime % 60, 10);
            var playedM = parseInt(audio.currentTime / 60);

            if(playedS < 10) playedS = '0' + playedS;

            elem.html(playedM + '.' + playedS);
        } else {
            elem.html('0.00');
        }
    }
    ...
    sections.first().addClass('is-active');
    ...
    tab.on('click', function () {
        ...
        player.find('*[data-tab]').removeClass('is-active');
        player.find('*[data-tab=' + dataTab + ']').addClass('is-active');
    });


    // определям порядковые номера треков
    // вставляем содержимое (атрибуты, длительность)
    sections.each(function () {

        $(this).find('.player-content_item').each(function(index) {
            var elem = $(this);
            var header = elem.find('h3');
            var audio = elem.find('audio')[0];

            // stuck solution
            // https://stackoverflow.com/questions/33185721/htmlmediaelement-duration-returning-nan-for-audio-file?noredirect=1&lq=1
            audio.addEventListener("loadedmetadata", function() {
                ...
                duration = m + '.' + s;
                elem.append('<span class="player_duration">' + duration + '</span>');
            });


            header.attr('data-title', header.text());
            header.prepend('<span class="player_track-number">' + (index + 1) + '.</span>');
        });
    });
    ...
    // добавление атрибута для audio preload="metadata"
    // https://stackoverflow.com/questions/44610417/audio-duration-returns-nan?rq=1
});